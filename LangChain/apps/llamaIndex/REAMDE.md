...
## 问题
### 问题1: 每个查询都会经过向量存储检索，然后根据检索到的文档上下文生成答案。没有机制来判断查询是否与文档相关。像“你是谁”这样的问题仍然会检索文档块（因为它们有最接近的嵌入），然后LLM会严格基于这些文档块进行回答。

> 解决办法: 构建一一个带有工具的 Agent： 一个用于文档查询的 QueryEngineTool —— 仅当用户询问文档内容时才使用,Agent 本身可以用自己的知识回答一般性问题（无需任何工具）