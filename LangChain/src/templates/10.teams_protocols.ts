...
class TeammateManager {
  ...
  async teammateLoop(name: string, role: string, prompt: string) {
    ...
    const sysPrompt = `你是一个名为 '${name}' 的队友，角色: ${role}，工作目录: ${WORKDIR}。在进行重大工作之前，请通过 plan_approval 提交计划。使用 shutdown_response 响应 shutdown_request。`;
    ...
    if (member && member.status !== "shutdown") {
      member.status = "idle";
      this.saveConfig();
      logStatus(`Status updated to idle`);
    }
  }
