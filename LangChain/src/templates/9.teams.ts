...
class TeammateManager {
  ...
  spawn(name: string, role: string, prompt: string): string {
    ...
    // 启动队友线程
    const thread = spawn(process.execPath, [__filename, name, role, prompt], {
      cwd: WORKDIR,
      detached: true,
      stdio: "ignore",
    });
    ...
    for (let i = 0; i < 50; i++) {
      ...
      for (const msg of inbox) {
        ...
        if (msg.type === "shutdown_request") {
          ...
          BUS.send(name, "lead", "shutdown acknowledged", "shutdown_response");
          log(`Shutdown request received, exiting`);
          console.log(`  [${name}] 收到关机请求，已停止工作`);
          return;
        }
